<x-base>
    ...
    <div class="hero-content flex-col lg:flex-row-reverse gap-[10vw]">
        ...
        <div class="card-body">
            <form action="{{ route('login') }}" method="POST" class="fieldset gapy-[1vw]">
                @csrf
            <label for="email" class="label">Email</label>
            <input name="email" required type="email" class="input w-full" value="{{ old('email') }}" placeholder="Email" />
            <label for="password" class="label">Password</label>
            <input name="password" required type="password" class="input w-full" placeholder="Password" />
            <button class="btn btn-neutral mt-4">Login</button>
            </form>

            <div class="mt-3">
                Don't have an account? <span class="text-blue-600/70"><a href="{{ route('register') }}">Register</a></span>
            </div>
